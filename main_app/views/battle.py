...
from ..models import Crazybone, Comment, Profile, Clan, Battle
...
@login_required
def create(req):
    def_cb = req.GET['defender-cb'].split('-')
    defender = User.objects.get(username=def_cb-db[0]).profile
    defender_cb = Crazybone.objects.get(name=def_cb-db[1])
    challenger_cb = Crazybone.objects.get(name=req.GET['challenger_cb'])
    Battle.objects.create(challenger=req.user.profile, defender=defender, challenger_cb=challenger_cb, defender_cb=defender_cb)
    return redirect('/battle/display')